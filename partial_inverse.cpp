...
// Pybind11 Module
PYBIND11_MODULE(partial_inverse, m) {
...
// PYBIND11_MODULE(pinv, m) {
//     m.doc() = "Simple module for computing the partial inverse of a scipy.sparse CSC matrix";

//     /*py::class_<Eigen::SparseMatrix<double>>(m, "SparseMatrix")*/
//     /*    .def(py::init<>())*/
//     /*    .def("rows", &Eigen::SparseMatrix<double>::rows)*/
//     /*    .def("cols", &Eigen::SparseMatrix<double>::cols);*/

//     m.def("pinv", [](py::object scipy_csc) {
//         using SpMat = Eigen::SparseMatrix<double>;
//         SpMat Q = py::cast<SpMat>(scipy_csc); // Convert scipy sparse to Eigen
//         return partial_inverse(Q);
//     }, "Compute the partial inverse of a scipy.sparse CSC matrix");
